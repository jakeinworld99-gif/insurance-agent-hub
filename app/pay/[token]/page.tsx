...
export default function PayPage() {
  ...
  useEffect(() => {
    const load = async () => {
      // Use service client approach: the payment token is the auth
      const { data: pay } = await supabase
        .from('payments').select('*, proposals(*, customers(*), products(*))')
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 space-y-6">
          ...
          <div className="space-y-3 text-gray-700">
            ...
            <div className="flex justify-between border-b pb-3">
              ...
              <span className="font-medium">Rs {Number(product?.sum_insured_inr).toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500 font-semibold">Amount Due</span>
              <span className="text-xl font-bold text-blue-600">
                Rs {Number((payment.amount_cents || 0) / 100).toLocaleString()}
              </span>
            </div>
          </div>
          ...
          >
            {paying ? 'Processing...' : `Pay Rs ${Number((payment.amount_cents || 0) / 100).toLocaleString()} (demo)`}
          </button>
