'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { createClient } from '@/lib/supabase/client'

export default function NewCustomerPage() {
  const router = useRouter()
  const supabase = createClient()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [form, setForm] = useState({
    full_name: '',
    customer_email: '',
    phone: '',
    date_of_birth: '',
    vehicle_type: '',
    occupation: '',
    annual_income: '',
    city: '',
  })

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    setError('')
    const res = await fetch('/api/agent/me')
    const { agentId } = await res.json()
    if (!agentId) { router.push('/login'); return }
    const { error } = await supabase.from('customers').insert({
      agent_id: agentId,
      full_name: form.full_name,
      customer_email: form.customer_email,
      phone: form.phone.replace(/^\+/, ''),
      date_of_birth: form.date_of_birth,
      vehicle_type: form.vehicle_type || null,
      occupation: form.occupation || null,
      annual_income: form.annual_income ? parseFloat(form.annual_income) : null,
      city: form.city || null,
    })
    if (error) { setError(error.message); setLoading(false) }
    else router.push('/dashboard')
  }

  const field = (key: string, label: string, type = 'text', required = false) => (
    <div key={key}>
      <label className="block text-sm font-medium text-gray-700 mb-1">{label}{required && ' *'}</label>
      <input
        type={type}
        required={required}
        value={form[key as keyof typeof form]}
        onChange={(e) => setForm({ ...form, [key]: e.target.value })}
        className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
      />
    </div>
  )

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-xl mx-auto px-8 py-10">
        <button onClick={() => router.back()} className="text-gray-500 hover:text-gray-700 mb-6">
          Back
        </button>
        <h1 className="text-2xl font-bold text-gray-900 mb-8">New Customer</h1>
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow p-8 space-y-5">
          {field('full_name', 'Full Name *', 'text', true)}
          {field('customer_email', 'Customer Email *', 'email', true)}
          {field('phone', 'Phone (E.164, e.g. 919876543210) *', 'tel', true)}
          {field('date_of_birth', 'Date of Birth *', 'date', true)}
          {field('vehicle_type', 'Vehicle Type', 'text')}
          <p className="text-xs text-gray-400 -mt-2">Only for vehicle insurance customers (car, bike, etc.)</p>
          {field('occupation', 'Occupation')}
          {field('annual_income', 'Annual Income (INR)', 'number')}
          {field('city', 'City')}
          {error && <p className="text-red-600 text-sm">{error}</p>}
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-blue-600 text-white rounded-xl font-semibold hover:bg-blue-700 transition disabled:opacity-50"
          >
            {loading ? 'Creating...' : 'Create Customer'}
          </button>
        </form>
      </div>
    </div>
  )
}
