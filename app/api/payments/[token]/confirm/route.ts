import { createClient } from '@/lib/supabase/server'
import { NextRequest, NextResponse } from 'next/server'
import { Resend } from 'resend'

export const runtime = 'nodejs'

export async function POST(req: NextRequest, { params }: { params: { token: string } }) {
  const supabase = await createClient()

  // Atomic single-shot: only the request that flips pending -> completed gets the row
  const { data: pay, error: flipError } = await supabase
    .from('payments')
    .update({ status: 'paid', paid_at: new Date().toISOString() })
    .eq('token', params.token)
    .eq('status', 'pending')
    .select('*, proposals(*, customers(*), products(*))')
    .single()

  if (flipError || !pay) {
    const { data: existing } = await supabase
      .from('payments').select('status').eq('token', params.token).single()
    if (!existing) return NextResponse.json({ error: 'Payment not found' }, { status: 404 })
    return NextResponse.json({ error: 'Payment already processed', status: existing.status, replay: true }, { status: 409 })
  }

  const proposal = pay.proposals
  const customer = proposal?.customers
  const product = proposal?.products

  // Update proposal status
  await supabase.from('proposals').update({ status: 'paid' }).eq('id', pay.proposal_id)

  // Create policy
  const policyNumber = `POL-${Date.now().toString(36).toUpperCase()}`
  await supabase.from('policies').insert({
    proposal_id: pay.proposal_id,
    customer_id: pay.proposals?.customer_id,
    product_id: pay.proposals?.product_id,
    agent_id: pay.proposals?.agent_id,
    policy_number: policyNumber,
    premium: pay.amount_cents,
    started_at: new Date().toISOString(),
    status: 'active',
  })

  // Send confirmation email via Resend
  let email: { sent: boolean; id?: string; error?: string } = { sent: false }
  const customerEmail = customer?.customer_email || customer?.email

  if (!customerEmail) {
    email = { sent: false, error: 'customer has no email on file' }
  } else if (!process.env.RESEND_API_KEY) {
    email = { sent: false, error: 'RESEND_API_KEY not configured' }
  } else {
    try {
      const resend = new Resend(process.env.RESEND_API_KEY)
      const { data, error } = await resend.emails.send({
        from: process.env.EMAIL_FROM || 'Insurance Platform <onboarding@resend.dev>',
        to: customerEmail,
        subject: `Payment confirmed — Policy ${policyNumber}`,
        html: `<p>Dear ${customer?.full_name},</p>
          <p>Your payment of Rs ${(pay.amount_cents / 100).toLocaleString()} has been received.</p>
          <p>Your policy <strong>${policyNumber}</strong> for ${product?.name} is now active.</p>
          <p>Thank you,<br/>Your Insurance Agent</p>`,
      })
      if (error) {
        email = { sent: false, error: error.message || String(error) }
      } else {
        email = { sent: true, id: data?.id }
      }
    } catch (e) {
      email = { sent: false, error: e instanceof Error ? e.message : String(e) }
    }
  }

  return NextResponse.json({
    success: true,
    policy_number: policyNumber,
    email,
  })
}
